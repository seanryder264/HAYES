...
void measure(void *pvParameters)
{
    ...
    measure_cell grid[SIZE_X * SIZE_Y];
    ...
    while (1) {
        for (uint8_t cell_y = 0; cell_y < SIZE_Y; cell_y ++) {
            for (uint8_t cell_x = 0; cell_x < SIZE_X; cell_x ++){
                ...
                for (uint8_t channel = 0; channel < 1; channel ++) {
                    ...
                    grid[cell_index].amplitude = (uint8_t)((amplitude / 1500000) * 255);
        ...
        sprintf(buffer, "ID: %u", grid[max_index].frequency);
...
void measure_collect(measure_group *measurements, uint8_t cell_x, uint8_t cell_y)
{
    ...
    */
    ...
    HAL_ADC_Start_DMA(&hadc1, (uint32_t *) measurements->samples[3].ADC_buffer, 128);
    HAL_ADC_Start_DMA(&hadc2, (uint32_t *) measurements->samples[2].ADC_buffer, 128);
    HAL_ADC_Start_DMA(&hadc3, (uint32_t *) measurements->samples[1].ADC_buffer, 128);
    HAL_ADC_Start_DMA(&hadc4, (uint32_t *) measurements->samples[0].ADC_buffer, 128);

    HAL_TIM_IC_Start_DMA(&htim16, TIM_CHANNEL_1, (uint32_t*) measurements->samples[0].IC_buffer, 2);
    HAL_TIM_IC_Start_DMA(&htim4 , TIM_CHANNEL_1, (uint32_t*) measurements->samples[1].IC_buffer, 2);
